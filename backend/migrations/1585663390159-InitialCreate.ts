...
export class InitialCreate1585663390159 implements MigrationInterface {
    name = 'InitialCreate1585663390159'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("CREATE TABLE `product` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `price` double NOT NULL, `amount` double NOT NULL, `description` text NULL, `createdDate` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `userId` int NULL, UNIQUE INDEX `IDX_22cc43e9a74d7498546e9a63e7` (`name`), PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
        await queryRunner.query("CREATE TABLE `purchase` (`id` int NOT NULL AUTO_INCREMENT, `amount` double NOT NULL, `price` double NOT NULL, `description` text NULL, `reduceStock` tinyint NOT NULL, `completed` tinyint NOT NULL, `deliveryDate` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `createdDate` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `customerId` int NULL, `productId` int NULL, `userId` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
        await queryRunner.query("CREATE TABLE `customer` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `address` varchar(200) NULL, `phone` varchar(50) NULL, `companyName` varchar(100) NULL, `taxId` varchar(20) NULL, `nationalId` varchar(20) NULL, `checkingAccount` varchar(40) NULL, `description` text NULL, `createdDate` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `userId` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
        ...
        await queryRunner.query("ALTER TABLE `purchase` ADD CONSTRAINT `FK_9af3a556aa0f166dd771a1e6c46` FOREIGN KEY (`productId`) REFERENCES `product`(`id`) ON DELETE CASCADE ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `purchase` ADD CONSTRAINT `FK_33520b6c46e1b3971c0a649d38b` FOREIGN KEY (`userId`) REFERENCES `user`(`id`) ON DELETE CASCADE ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `customer` ADD CONSTRAINT `FK_3f62b42ed23958b120c235f74df` FOREIGN KEY (`userId`) REFERENCES `user`(`id`) ON DELETE CASCADE ON UPDATE NO ACTION", undefined);
    }
